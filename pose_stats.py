...
class DataRepresentation:
    ...
    def pose_stats(self, list):

        path = "/home/jayantdubey/Desktop/AllData"
        testpath = "/home/jayantdubey/Desktop/Alldata"
        ...
        for i in range(len(io_pair)):
            ...
            translation = self.target_translation[self.start:self.end, :].cpu().numpy()   # [N, 3]
            quat = self.target_quaternion[self.start:self.end, :].cpu().numpy()  # [N, 4]
            euler = R.from_quat(quat).as_euler('yzx', degrees=True)
            

            print(f"Translation Mean: {np.mean(translation,axis=0)} mm std: \n min/max Y: {np.min(translation[:,1])}mm,{np.max(translation[:,1])}mm,| min/max Z: {np.min(translation[:,1])}mm {np.max(translation[:,0])}mm, | min/max X: {np.min(translation[:,2]) }mm, {np.max(translation[:,2])}mm,")
            
            print(f"Euler mean \n Ydeg: {np.mean(euler[:, 0])} | Zdeg: {np.mean(euler[:, 1])} | Xdeg: {np.mean(euler[: ,2])}")
            print(f"Euler std \n Ydeg: {np.std(euler[:, 0])} | Zdeg: {np.std(euler[:, 1])} | Xdeg: {np.std(euler[: ,2])}")

            list.append(np.array([np.mean(translation,axis=0), np.std(translation,axis=0)]))
